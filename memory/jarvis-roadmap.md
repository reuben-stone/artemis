...
WORKER MODEL DECOUPLED (2026-06-26, cost fix): dispatch_worker hit a $10 Anthropic credit wall after ~1.5 workers — a worker is a FULL agent loop (worker.ts, up to MAX_ROUNDS=30 tool rounds, max_tokens 8096, growing context), and it was inheriting the CHAT model (getModelClient→getModel) on the metered API, so Opus or large-context runs = $$$. Fix: store.getWorkerModel/setWorkerModel (meta key 'worker_model', DEFAULT 'claude-sonnet-4-6'); getModelClient now takes opts.model override; worker.ts calls getModelClient({model:getWorkerModel()}); IPC agent:get/setWorkerModel + preload bindings; Settings→Model "Worker model" seg (Haiku/Sonnet/Opus, self-fetched). So workers stay cheap even when chat is on Opus. Honest levers also noted to Reuben: dispatch one at a time (watch cost), and the endgame = route workers to LOCAL ollama once on the new Mac (free) — the model seam already supports per-call backend. ARTEMIS-CORE dispatch_worker section updated with the cost note.
