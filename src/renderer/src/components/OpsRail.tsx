...
/**
 * The ops section of the HUD rail — the mission anchor that sits above the personal day
 * planner. Two glanceable cards backed by data Artemis already gathers:
 ...
 * Each card's expand button opens the existing full dock — the seam the agent-driven-UI
 * step will drive next.
 */
...
export function OpsRail({
  ...
  // Load the ecosystem snapshot once per session (cached), not on the turn loop.
  useEffect(() => {
    if (!ecoCache) void loadEco()
  }, [loadEco])
